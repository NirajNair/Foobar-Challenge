...
Write a function called solution(n) that takes a positive integer n and returns the number of different staircases
 that can be built from exactly n bricks. n will always be at least 3 (so you can have a staircase at all), but no more than 200,
...
def solution(n):
    ...
    print(count(n, 1, memo) - 1)


def count(bricks_left, height, memo):
    if memo[bricks_left][height] != -1:
        return memo[bricks_left][height]
    
    if bricks_left == 0 or bricks_left == height:
        return 1
    
    if bricks_left < height:
        return 0
    res = count(bricks_left - height, height+1, memo) + count(bricks_left, height+1, memo)
    memo[bricks_left][height] = res
    return res
